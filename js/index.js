...
$(function(){
    ...
    __GetIP();
    __GetInfo();
    console.log("%c🌳 Name：PhamHoangGiaBao | Bio：Sayy Hello | Url：https://www.giabao.com 🌳", "font-family: ;color:#fff; background: linear-gradient(270deg, #18d7d3, #68b7dd, #8695e6, #986fee); padding: 8px 15px; border-radius: 8px");
    console.log("…………./´¯/)……….. (\\¯’\\\n…………/….//……….. …\\\\….\\\n………../….//………… ….\\\\….\\\n…../´¯/…./´¯\\\\………../¯‘\\\\…./¯’\\\\\n.././…/…./…./.|_|……_| .\\\\…\\\\…\\\\…\\\\.\\\\\n(.(….(….(…./.)..)..(..(. \\\\….)….)….).)\n.\\\\…………….\\/…/…\\\\. ..\\/……………./\n..\\\\…………….. /……..\\\\………………../\n…\\\\…………..(………. ..)……………./");
  });
// CHECK FPS
...
// MUSIC PLAY
const MusicList=[
    "https://files.catbox.moe/4vitay.mp3",
];
...
function ShowToast() {
  ...
  $(document).ready(function() {
    ...
    $('#Facebook').click(function() {
        OpenUrl('https://www.facebook.com/share/1EeMYLbtk3/?mibextid=wwXIfr');
    });
      // SET URL YOUTUBE
    $('#Youtube').click(function() {
        OpenUrl('https://youtu.be/cizoIav3geY?si=BImgRRCK6bIEUUFk');
    });
    // SET URL DISCORDS
    $('#Discord').click(function() {
        OpenUrl('https://dsc.bio/phgb.0310');
    });
      // SET URL INSTAGRAM
    $('#Instagram').click(function() {
        OpenUrl('https://www.instagram.com/_phgb.dev._?igsh=MWlxOTgwcXpubjQ2MA%3D%3D&utm_source=qr');
    });
  });
...
function Alerts() {
  Swal.fire({
    title: '𝙉𝙤𝙩𝙞𝙛𝙞𝙘𝙖𝙩𝙞𝙤𝙣 !',
    text: '=>>  𝙉𝙝𝙖̣̂𝙥 𝙣𝙜𝙤𝙖̀𝙞 𝙢𝙖̀𝙣 𝙝𝙞̀𝙣𝙝 đ𝙚̂̉ 𝙩𝙖̆́𝙩 🐬 <<=',
...
let defaultViews = 1650;
// Like (tăng khi bấm nút)
let likes = localStorage.getItem('profile_likes') || 0;
if(likeCount) likeCount.innerText = likes;

if(likeBtn){
  likeBtn.addEventListener('click', () => {
    likes = parseInt(likes) + 1;
    localStorage.setItem('profile_likes', likes);
    likeCount.innerText = likes;
  });
}
// Tăng view
// Lấy số view hiện tại trong localStorage
let views = localStorage.getItem('profile_views');
if(!views) {
  // Nếu chưa có trong localStorage, dùng số mặc định
  views = defaultViews;
} else {
  views = parseInt(views);
}

// Tăng 1 khi có người truy cập
views = views + 1;
localStorage.setItem('profile_views', views);

// Hiển thị trên trang
if(viewCount) viewCount.innerText = views;